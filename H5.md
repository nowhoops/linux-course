...
## The way it was should have been done
### a)
```bash
- I created a configuration file at:  
sudo mkdir /etc/apache2/sites-available/heema.fi.conf
```
- Then I created the directory structure defined in the configuration:
- ```bash 
`mkdir -p /home/user/public-sites/heema.fi
```

Inside that folder, I created an `index.html` file using the nano editor and inserted a valid HTML template, removing unnecessary parts.

- I enabled the site with Apache using:  
```bash
sudo systemctl reload apache2
sudo a2ensite heema.fi
```
- Tested the site with `curl`
