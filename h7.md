...
while true; do
  ...
  if [[ ! "$username" =~ ^[a-z][-a-z0-9_]*$ ]]; then
    echo "Invalid username. Use only lowercse letters, numbers, dashes or underscores."
    continue
  fi
  ...
  USER_WEB_DIR="$WEBROOT/$username.heema.fi"
  sudo mkdir -p "$USER_WEB_DIR"

  # Create the HTML page (it is old from the task  H3)
  # It only changes the name in the middle
...
EOF
  ...
  sudo tee "$APACHE_CONF_DIR/$username.heema.fi.conf" > /dev/null <<EOF
<VirtualHost *:80>
    ServerName $username.heema.fi
    DocumentRoot $USER_WEB_DIR
...
EOF
  ...
  sudo a2ensite "$username.heema.fi.conf"
  sudo systemctl reload apache2

  echo "Website ready at: http://$username.heema.fi"
  echo "If DNS isn't configured, add this to your /etc/hosts file:"
  echo "127.0.0.1 $username.heema.fi"
